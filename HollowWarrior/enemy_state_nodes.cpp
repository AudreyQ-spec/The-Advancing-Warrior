...
void EnemyDashInAirState::on_enter()
{
	...
	Vector2 post_target = { player->get_position().x,player->get_floor_y() };
	enemy->set_velocity((post_target - enemy->get_position()).normalize() * SPEED_DASH);
...
{
	MessageBox(GetHWnd(), _T("VICTORY! 孩子你无敌了！"), _T("挑战成功! "), MB_OK);
	exit(0);
}
